...
# 主要内容
# 增加了使用测试工具对模型进行测试，并记录测试结果 model.score()
# mean_squared_error 均方误差
# r2_score 平均系数
# 又增加了一种模型xgb.XGBRegressor(n_extimator=100, ...)
...
# 将分类（字符串）列转换为0/1 的哑变量
X = pd.get_dummies(X)  # 将分类变量转为数值变量

# 将年龄段转为数值变量
# y = pd.to_numeric(lambda x: int(x.split(" ")[0]))  # 假设年龄段为整数
# 正确写法
...
# XBGRegressor 高度优化且易于集成到现有流水线（pipeline）中的模型
# fit => predict => evalute
...
with open("2.2.3_report_xgb.txt", "w") as xgb_report_file:
    ...
    xgb_report_file.write(
        f"XGBoost均方误差(MSE): {mean_squared_error(y_test, y_pred_xgb)}\n"
    )
