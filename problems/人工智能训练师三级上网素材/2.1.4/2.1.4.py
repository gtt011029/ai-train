...
# 增加诊断延迟和病程列
# .dt.days 是pandas处理时间差（timedelta）的核心属性， 直接提取两个日期之间相隔的整数天数
...
treatment_outcome_distribution = (
    data.groupby("疾病类型")["治疗结果"].value_counts().unstack()
)
...
# 绘制柱状图
# plot是pandas dataFrame的绘图方法，直接把表格数据化成图表
# stacked：变成堆叠柱状图
...
# 绘制散点图
# scatter 是matplotlib的散点图函数，把两组数据画成一个个独立的点
